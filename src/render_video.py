...
def ffmpeg_filter_for(files, shot_seconds):
    parts = []
    labels = []
    frames = max(1, int(round(shot_seconds * FPS)))
    fade = min(0.18, shot_seconds / 8)
    ...
    for i, path in enumerate(files):
        ...
        if path.suffix.lower() in IMAGE_EXTS:
            chain = (
                ...
                f"crop={W}:{H},"
                f"zoompan=z='min(zoom+0.00055,1.10)':"
                f"x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':"
                f"d={frames}:s={W}x{H}:fps={FPS},"
                f"trim=duration={shot_seconds},setpts=PTS-STARTPTS,"
                f"eq=contrast=1.03:saturation=1.08,"
                f"fade=t=in:st=0:d={fade},"
                f"fade=t=out:st={max(0, shot_seconds-fade)}:d={fade},setsar=1[{label}]"
            )
        else:
            chain = (
                f"[{i}:v]trim=duration={shot_seconds},setpts=PTS-STARTPTS,"
                f"scale={W}:{H}:force_original_aspect_ratio=increase,crop={W}:{H},"
                f"eq=contrast=1.03:saturation=1.08,fps={FPS},"
                f"fade=t=in:st=0:d={fade},"
                f"fade=t=out:st={max(0, shot_seconds-fade)}:d={fade},setsar=1[{label}]"
            )
        parts.append(chain)
    ...
    parts.append(
        "".join(labels)
        + f"concat=n={len(files)}:v=1:a=0,trim=duration={duration},setpts=PTS-STARTPTS[base]"
    )
...
def main():
    ...
    needed = max(1, int(audio_duration / DEFAULT_SHOT_SECONDS) + 1)
    files = files[:needed]
    ...
    cmd += [
        ...
        "-pix_fmt", "yuv420p", "-movflags", "+faststart", str(base),
    ]
    ...
    subtitle_filter = (
        ...
        "BorderStyle=1,Outline=3,Shadow=1,Alignment=2,MarginV=250'"
    )
    subprocess.run([
    ...
    # Normalize loudness during the final mux so every generated Short has a
    # consistent listening level without another video encode.
    subprocess.run([
        "ffmpeg", "-y", "-i", str(captioned), "-i", str(audio),
        "-map", "0:v:0", "-map", "1:a:0",
    ...
    if final_duration < 10 or final_duration > 46:
        raise RuntimeError(f"Rendered video duration is outside Shorts bounds: {final_duration:.2f}s")
    print(f"Created {output} ({final_duration:.1f}s) with motion, captions, normalized audio, and vertical framing")
