...
Zero-cost by default: no paid image API is required. The engine discovers
local reference assets, fingerprints them, and emits provider-neutral metadata
for image/I2V backends. Kids-animation engines are intentionally untouched.
"""
...
def build(bible: dict, plan: dict) -> dict:
    ...
    for character in bible.get("characters", []):
        ...
        cid = str(character.get("id") or safe_name(name))
        ...
        by_id[cid.lower()] = entry
        by_id[name.lower()] = entry

    scene_refs = []
    for scene in plan.get("scenes", []):
        subject = str(scene.get("subject", ""))
        subject_id = str(scene.get("character_id", ""))
        matched = by_id.get(subject_id.lower()) or by_id.get(subject.lower())
        scene_refs.append({
            ...
            "continuity_instruction": "Preserve immutable identity and wardrobe traits. Use image conditioning when available; otherwise preserve the canonical prompt and mark the scene prompt-only.",
        })
