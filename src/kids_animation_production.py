...
def make_background(kind: str, path: Path) -> None:
    ...
    if kind == "bedroom":
        img.paste((235,220,190), [0,0,WIDTH,HEIGHT]); d.rectangle([0,480,WIDTH,HEIGHT], fill=(215,190,150)); d.rectangle([120,260,520,480], fill=(120,160,205), outline=(70,100,145), width=8); d.ellipse([780,90,980,290], fill=(255,215,90))
    else:
        ...
        if kind in {"garden","flower","water","friends","dance"}:
            for x in range(100,1200,130):
                y=500+(x%90); d.line([x,y,x,y+55],fill=(45,125,55),width=6); d.ellipse([x-15,y-15,x+15,y+15],fill=(245,120,170))
...
def make_character(path: Path) -> None:
    ...
    d.ellipse([35,80,145,245],fill=(145,92,48),outline=(95,60,35),width=6); d.ellipse([275,80,385,245],fill=(145,92,48),outline=(95,60,35),width=6)
    d.ellipse([100,230,320,405],fill=(185,125,65),outline=(105,70,40),width=7); d.ellipse([75,55,345,310],fill=(205,145,78),outline=(105,70,40),width=7); d.ellipse([145,175,275,265],fill=(238,190,125)); d.ellipse([196,178,225,205],fill=(45,35,30))
...
def make_flower(path: Path) -> None:
    ...
    for a in range(0,360,72):
        r=math.radians(a); cx,cy=110+int(math.cos(r)*65),120+int(math.sin(r)*65); d.ellipse([cx-30,cy-30,cx+30,cy+30],fill=(245,105,170))
    img.save(path)
...
def scene(index: int, kind: str, line: str) -> Path:
    bg,char,flower=assets(kind); sd=SCENES/f"scene_{index:03d}"; sd.mkdir(parents=True,exist_ok=True); wav=sd/"voice.wav"; voice(line,wav); silent=sd/"silent.mp4"; out=sd/"scene.mp4"
    sx,ex=((-80,930) if index%2 else (930,-80)); inputs=["-loop","1","-i",str(bg),"-loop","1","-i",str(char)]
    flower_input=None
    ...
    if flower_input is not None:
        filt=f"[0:v]zoompan=z='min(zoom+0.0008,1.06)':d={FPS*SCENE_SECONDS}:s={WIDTH}x{HEIGHT}:fps={FPS}[bg];[1:v]format=rgba,scale=360:-1[m];[2:v]format=rgba,scale=180:-1[f];[bg][m]overlay=x='{sx}+({ex}-{sx})*(t/{SCENE_SECONDS})':y='390+12*sin(t*3)'[x];[x][f]overlay=x='900+15*sin(t*2)':y='390+8*sin(t*3)':enable='between(t,1.5,{SCENE_SECONDS})'[v]"
    else:
        filt=f"[0:v]zoompan=z='min(zoom+0.0008,1.06)':d={FPS*SCENE_SECONDS}:s={WIDTH}x{HEIGHT}:fps={FPS}[bg];[1:v]format=rgba,scale=360:-1[m];[bg][m]overlay=x='{sx}+({ex}-{sx})*(t/{SCENE_SECONDS})':y='390+12*sin(t*3)'[v]"
...
def main() -> int:
    content=os.getenv("CONTENT_TYPE","poem").strip().lower(); duration=float(os.getenv("DURATION","1"))
